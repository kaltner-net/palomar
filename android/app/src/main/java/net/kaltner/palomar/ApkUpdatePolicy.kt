...
internal const val PALOMAR_APPLICATION_ID = "net.kaltner.palomar"
internal const val OFFICIAL_APK_RELEASE_SOURCE_LABEL = "Official Palomar GitHub releases"
internal const val OFFICIAL_RELEASES_API = "https://api.github.com/repos/kaltner-net/palomar/releases/tags/"
internal const val OFFICIAL_RELEASE_DOWNLOAD_PREFIX =
    "https://github.com/kaltner-net/palomar/releases/download/"
...
internal fun selectAndroidReleaseAssets(
    ...
    if (
        ...
        target.releaseNotesUrl != "https://github.com/kaltner-net/palomar/releases/tag/$tag"
    ) {
    ...
    val expectedApk = "palomar-$tag.apk"
    val expectedArchive = "palomar-linux-$tag.tar.gz"
    val expectedNames =
        setOf(
            expectedApk,
            expectedArchive,
    ...
    if (names.toSet() != expectedNames || names.size != expectedNames.size) {
        ...
        apkValidationFailure(code, "The Android release does not contain the exact expected Palomar asset set.")
    }
    ...
    return AndroidReleaseAssets(
        ...
        checksumManifest = asset("palomar-SHA256SUMS", MAX_RELEASE_METADATA_BYTES),
        checksumSignature = asset("palomar-SHA256SUMS.sig", MAX_RELEASE_METADATA_BYTES),
        releaseCertificate = asset("palomar-release-cert.pem", MAX_RELEASE_METADATA_BYTES),
    )
}
...
internal fun verifySignedReleaseManifest(
    ...
    val pinned = normalizedSha256(expectedCertificateFingerprint)
        ?: apkValidationFailure("invalidTrustAnchor", "This Palomar build has an invalid update trust anchor.")
    val installed = normalizedSha256(installedCertificateFingerprint)
        ?: apkValidationFailure("invalidInstalledSigner", "The installed Palomar signing identity is invalid.")
    val certificate = try {
    ...
    if (certificateFingerprint != pinned || certificateFingerprint != installed) {
        apkValidationFailure("releaseCertificateMismatch", "The release signing certificate does not match this Palomar installation.")
    }
...
internal fun validateDownloadedApkIdentity(
    ...
    if (downloaded.packageName != PALOMAR_APPLICATION_ID || installed.packageName != PALOMAR_APPLICATION_ID) {
        apkValidationFailure("packageMismatch", "The downloaded package is not the Palomar Android app.")
    }
    ...
    if (installed.signingCertificateSha256 != trusted || downloaded.signingCertificateSha256 != trusted) {
        apkValidationFailure("apkSignerMismatch", "The downloaded APK signing identity does not match installed Palomar.")
    }
    ...
    val installedSemVer = parseSemVer(installed.versionName)
        ?: apkValidationFailure("installedVersionInvalid", "The installed Palomar version cannot be updated automatically.")
    val targetSemVer = parseSemVer(targetVersion)
        ?: apkValidationFailure("targetVersionInvalid", "The selected Palomar release version is invalid.")
    if (targetSemVer == installedSemVer || downloaded.versionCode == installed.versionCode) {
        apkValidationFailure("sameVersion", "Palomar does not reinstall the same Android app version.")
    }
    if (targetSemVer < installedSemVer || downloaded.versionCode < installed.versionCode) {
        apkValidationFailure("downgrade", "Palomar does not install Android app downgrades.")
    }
    if (targetSemVer <= installedSemVer || downloaded.versionCode <= installed.versionCode) {
