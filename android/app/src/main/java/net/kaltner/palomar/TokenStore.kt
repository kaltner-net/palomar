...
internal fun suggestedHostDisplayName(host: String): String =
    if (host.trim().lowercase() in setOf("localhost", "127.0.0.1", "::1")) {
        ...
        host.trim().ifBlank { "Palomar host" }
    }
...
class HostStore(private val context: Context) {
    ...
    private val alias = "palomar_device_token"
    ...
    companion object {
        private const val CONNECTION_PREFERENCES = "palomar_connection"
        private const val LEGACY_UI_PREFERENCES = "palomar_preferences"
        private const val HOSTS_KEY = "hosts.v2"
        private const val ACTIVE_HOST_KEY = "activeHostId"

        fun preferenceFile(hostId: String) = "palomar_preferences.$hostId"
    }
}
...
class PreferenceStore(context: Context, hostId: String?) {
    private val preferences =
        context.getSharedPreferences(
            hostId?.let(HostStore::preferenceFile) ?: "palomar_preferences",
    ...
    internal fun loadServerUpdateOperationId(): String? =
        preferences.getString("serverUpdateOperation.v1", null)
            ?.takeIf { it.matches(Regex("^pmu_[A-Za-z0-9_-]{16,80}$")) }

    internal fun setServerUpdateOperationId(operationId: String) {
        if (operationId.matches(Regex("^pmu_[A-Za-z0-9_-]{16,80}$"))) {
