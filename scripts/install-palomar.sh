...
RELEASES_API="https://api.github.com/repos/kaltner-net/palomar/releases"
...
fail() {
  echo "install-palomar: $*" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: install-palomar.sh [--version vMAJOR.MINOR.PATCH]

Without --version, installs the newest complete stable Palomar release.
EOF
}
...
umask 077
temporary="$(mktemp -d "${TMPDIR:-/tmp}/palomar-bootstrap.XXXXXX")" \
...
fetch() {
  ...
  if ! curl --disable --silent --show-error --location \
    ...
    --header 'User-Agent: Palomar-bootstrap-installer' \
...
python3 - "$metadata" "$requested_version" "$DOWNLOAD_PREFIX" >"$tag_file" <<'PY' \
  || fail "GitHub release metadata has no complete stable Palomar release"
import json
import re
import sys
...
def complete_stable(release):
    ...
    if set(found) != expected:
        return None
    if found[f"palomar-linux-{tag}.tar.gz"] > 512 * 1024 * 1024:
        return None
    if any(found[name] > 64 * 1024 for name in ("palomar-SHA256SUMS", "palomar-SHA256SUMS.sig", "palomar-release-cert.pem")):
        return None
...
archive_name="palomar-linux-$tag.tar.gz"
archive="$temporary/$archive_name"
manifest="$temporary/palomar-SHA256SUMS"
signature="$temporary/palomar-SHA256SUMS.sig"
certificate="$temporary/palomar-release-cert.pem"
release_base="$DOWNLOAD_PREFIX/$tag"

fetch "$release_base/palomar-SHA256SUMS" "$manifest" "$MAX_SMALL_ASSET_BYTES" "palomar-SHA256SUMS"
fetch "$release_base/palomar-SHA256SUMS.sig" "$signature" "$MAX_SMALL_ASSET_BYTES" "palomar-SHA256SUMS signature"
fetch "$release_base/palomar-release-cert.pem" "$certificate" "$MAX_SMALL_ASSET_BYTES" "release certificate"
...
[ "$actual_cert_sha256" = "$TRUSTED_CERT_SHA256" ] \
  || fail "release signing certificate does not match Palomar's pinned identity"
...
openssl dgst -sha256 -verify "$public_key" -signature "$signature" "$manifest" >/dev/null 2>&1 \
  || fail "palomar-SHA256SUMS signature verification failed"
...
with open(archive_path, "rb") as handle:
    for chunk in iter(lambda: handle.read(1024 * 1024), b""):
        digest.update(chunk)
if digest.hexdigest() != checksums[f"palomar-linux-{tag}.tar.gz"]:
    raise SystemExit(1)
PY
...
required = {
    "install.sh", "palomar-release.properties", "requirements.txt",
    "linux/palomar", "linux/palomar.service", "linux/palomar-update-recovery.service",
    "linux/palomar_service.py", "linux/codex.py", "linux/approvals.py",
    ...
    "linux/server_update.py", "linux/update_cli.py", "linux/palomar_updater.py",
    "linux/palomar_uninstall", "LICENSE", "NOTICE", "THIRD_PARTY_NOTICES.md",
    "linux/completions/palomar.bash", "linux/completions/_palomar",
...
properties = {}
try:
    for raw_line in (destination / "palomar-release.properties").read_text(encoding="utf-8").splitlines():
        line = raw_line.strip()
    ...
    raise SystemExit(1) from error
if (
    properties.get("palomarVersion") != tag[1:]
...
echo "Resolved Palomar $tag from $SOURCE."
echo "Running the verified bundled installer..."
bash "$payload/install.sh"

echo
echo "Palomar $tag was installed from $SOURCE."
echo "Next steps:"
echo "  palomar status"
echo "  palomar pair"
echo "  palomar web"
