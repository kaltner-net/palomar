...
This document is the architecture and security decision for Palomar server
updates. It is normative for `palomar update`, the web client, Android, the
release workflow, and the external updater helper.

## Trust and release discovery

Palomar has one stable update channel: published, non-prerelease releases in
`kaltner-net/palomar`. Discovery uses fixed GitHub API paths and accepts only a
...
1. the release exposes exactly one nonempty Linux archive, `palomar-SHA256SUMS`,
   `palomar-SHA256SUMS.sig`, and `palomar-release-cert.pem` with canonical download
   URLs for the discovered tag;
2. the certificate's DER SHA-256 fingerprint equals the trust anchor in the
   currently installed `palomar-release.properties`;
3. OpenSSL validates the detached SHA-256 signature over the exact
   `palomar-SHA256SUMS` bytes with that certificate;
...
6. the staged `palomar-release.properties` identifies the discovered version, is an
...
The operation record is atomically replaced in the private Palomar state
...
fails without activation. Palomar never interrupts work to make an update
...
installed at `~/.local/libexec/palomar-updater`. The helper, not the Palomar
...
in a separate transient unit so restarting `palomar.service` cannot kill it.
...
The installer also enables `palomar-update-recovery.service`, a fixed oneshot
...
`palomar.service` have no ordering dependency: it can stop a concurrently
started Palomar, restore or finish the payload, restart it, and perform the same
...
backoff and requires the target Palomar version and protocol. Success commits
the update and removes downloaded, staged, and backup payloads while retaining
the bounded operation record.

Activation or health failure triggers rollback: stop only `palomar.service`,
...
clients show `palomar update --recover` plus the manual recovery procedure
...
1. Run `palomar update --recover` and then `palomar status`.
2. If the helper cannot recover, inspect the operation with
   `palomar update --status`; do not delete its backup.
...
4. If service activation still fails, use `journalctl --user -u
   palomar.service`; never paste tokens or `palomar.env` into an issue.
