...
The Palomar service runs on Linux under a systemd user manager. Before
installing, provide:
...
that will run Palomar. Palomar never installs or authenticates either CLI.
...
curl -fsSL https://raw.githubusercontent.com/kaltner-net/palomar/main/scripts/install-palomar.sh -o "$install_tmp/install-palomar.sh"
less "$install_tmp/install-palomar.sh"
sh "$install_tmp/install-palomar.sh"
rm -rf -- "$install_tmp"
```
...
The [GitHub release page](https://github.com/kaltner-net/palomar/releases) exposes
the versioned Linux archive plus `palomar-SHA256SUMS`, `palomar-SHA256SUMS.sig`, and
`palomar-release-cert.pem`. A manual installation must download the exact five
...
```sh
git clone https://github.com/kaltner-net/palomar.git
cd palomar
./install.sh
```
...
```sh
release_tag="$(curl -fsSL https://api.github.com/repos/kaltner-net/palomar/releases/latest | python3 -c 'import json, sys; print(json.load(sys.stdin)["tag_name"])')"
...
preparation aborts before Palomar replaces an installation. When Codex is also
...
CLI; Palomar pairing does not authenticate either provider. Check Claude adapter
detection with `palomar claude-status`.

The rootless installer copies Palomar to `~/.local/share/palomar`, installs the
CLI in `~/.local/bin`, installs Bash, Zsh, and Fish completions under
`~/.local/share`, installs and starts `palomar.service`, enables the
boot-time `palomar-update-recovery.service` oneshot, and creates:

```text
~/.config/palomar/palomar.env
~/.local/state/palomar
```
...
last login session ends. Enable lingering once so Palomar starts at boot and
...
may require administrator privileges; Palomar's installer remains rootless and
does not invoke `sudo`.

Reinstalling validates a staged copy before replacing the running installation.
An older `~/.local/share/palomar/venv` is retained through service activation
and removed only after the system-Python service starts successfully.
The installer never replaces `~/.config/palomar/palomar.env` or
`~/.local/state/palomar`, so configuration and paired-client tokens survive a
...
and rerun that release's `install.sh`. Confirm `palomar status`, `palomar web`,
...
```sh
PALOMAR_HOST=0.0.0.0
PALOMAR_PORT=8765
PALOMAR_WEB_HOST=0.0.0.0
PALOMAR_WEB_PORT=8766
PALOMAR_REMOTE_RESTART=0
PALOMAR_REPOSITORY_ROOT=/home/you/projects
# Present only when Codex was detected:
PALOMAR_CODEX_EXECUTABLE=/absolute/path/to/codex
# Optional:
PALOMAR_CLAUDE_EXECUTABLE=/absolute/path/to/claude
PALOMAR_NODE_EXECUTABLE=/absolute/path/to/node
PALOMAR_CODEX_SOCKET=/absolute/path/to/app-server-control.sock
PALOMAR_CODEX_FALLBACK_SOCKET=/absolute/path/to/palomar-codex-app-server.sock
# Comma-separated origins only when a reverse proxy uses a different origin:
PALOMAR_WEB_ORIGINS=https://palomar.example.internal
```

`PALOMAR_CODEX_SOCKET` is attach-only. Palomar never starts or replaces a
listener there. The fallback defaults to
`~/.local/state/palomar/codex-app-server.sock` and cannot provide Desktop live
co-presence.

Leave `PALOMAR_REMOTE_RESTART=0` unless authenticated clients should be able to
restart Palomar. Setting it to `1` enables only `palomar.service` restart through
...
After edits, use `palomar restart`. `palomar logs` follows the user journal.
`palomar web` prints the configured local web URL; replace `localhost` with the
host's trusted-LAN address when connecting remotely. The web client asks for
the same one-time `palomar pair` code and stores its persistent device token in
...
Android and web clients may save multiple independent Palomar installations.
...
| Command | Effect |
| --- | --- |
| `./linux/palomar install` | From a source checkout, run the transactional source installer. |
| `palomar start` | Start the Android TCP and browser HTTP/WebSocket listeners. |
| `palomar stop` | Stop both listeners. |
| `palomar restart` | Restart both listeners after an update or configuration change. |
| `palomar update --check` | Check the official stable channel without changing the installation. |
| `palomar update` | Review and start a signed, session-safe, recoverable server update. |
| `palomar update --status` | Show the latest durable update operation after reconnect or restart. |
| `palomar update --recover` | Retry restoration of the latest retained backup. |
| `palomar status` | Show the systemd user-service status. |
| `palomar logs` | Follow the service journal. |
| `palomar web` | Print the browser URL; it does not start a second web process. |
| `palomar pair` | Create a single-use pairing code valid for ten minutes. |
| `palomar uninstall` | Remove the service, CLI, updater, and application payload while retaining configuration and pairing/state data. |
| `palomar uninstall --purge` | Also remove Palomar configuration, pairing/state data, cache, and runtime data after confirmation. |

`palomar update --check` exits `0` when current, `10` when an update is
available, and `20` when active work blocks activation. `palomar update` shows
...
through `palomar update --status` after the expected disconnect.
...
needed. Run `./install.sh`, verify `palomar status`, open the URL from
`palomar web`, and enter a fresh code from `palomar pair`. The installed,
...
new Palomar-managed query resumes the exact session. Claude has no client
...
Android lifecycle alerts cover only monitored Palomar-managed Claude queries;
...
refresh, copy, and—when enabled—a confirmed Palomar restart. Android exposes the
...
**Settings → About**. This is separate from **Review server update**. Palomar
...
that finish, fail, are interrupted, or need attention while Palomar remains
...
overlay. Palomar authenticates both transports but doesn't terminate TLS. Do
not expose either port directly to the public Internet. Prefer Tailscale or
WireGuard; for browser TLS, place Palomar behind a trusted same-origin reverse
proxy and add its exact HTTPS origin to `PALOMAR_WEB_ORIGINS`. Tokens must never
be placed in proxy URLs or query strings.

## Android phone

Download the signed release APK from the matching tagged GitHub release,
sideload it, and run `palomar pair` immediately before setup. For development,
open `android/` in current Android Studio (compile SDK 37) and build the debug
app. The release APK embeds Palomar's `LICENSE` and
...
restarting Palomar reconnects with that token and reloads authoritative provider
...
**Resumable · Not live-attached**. **Resume in Palomar** starts a new managed
query on the same Claude session ID; Palomar cannot view current external events,
...
for active turns** in Palomar's settings and grant Android's notification
...
Remove the installed service, CLI, updater, and application payload while
retaining configuration and pairing/state data:

```sh
palomar uninstall
```

To remove Palomar configuration, device tokens, update state, cache, and runtime
data as well, use `palomar uninstall --purge`. Both forms ask for confirmation;
add `--yes` only in reviewed local automation. The command never touches a
source checkout or another application's files.
