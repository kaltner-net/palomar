#!/usr/bin/env python3
"""Palomar: a small authenticated TCP bridge to local Codex sessions."""
...
class Palomar:
    def __init__(
        ...
        self.claude_account_usage: dict[str, Any] = {
            "available": False,
            "experimental": True,
            "availabilityReason": "Updates after a Palomar-managed Claude run begins.",
        }
        for provider in PROVIDER_ORDER:
            cached_usage = self.state.provider_account_usage(provider)
            if not isinstance(cached_usage, dict):
                continue
            cached_snapshot = rate_limit_snapshot(cached_usage.get("rateLimits"))
            cached_observed_at = token_count(cached_usage.get("observedAt"))
            ...
            if provider == "codex":
                self.account_usage = restored
            else:
                self.claude_account_usage = {**restored, "experimental": True}
        ...
        metadata_path = install_directory / "palomar-release.properties"
        if not metadata_path.is_file():
            metadata_path = install_directory.parent / "palomar-release.properties"
        ...
        self.update_manager = update_manager or ServerUpdateManager(
            ...
            launcher_file=Path.home() / ".local/bin/palomar",
            unit_file=Path.home() / ".config/systemd/user/palomar.service",
            helper_file=Path.home() / ".local/libexec/palomar-updater",
            current_version=PALOMAR_VERSION,
            release_build=PALOMAR_RELEASE_BUILD,
    ...
    async def start(self) -> None:
        ...
        if self.control_path.exists():
            if not self.control_path.is_socket():
                raise RuntimeError("Palomar control socket path is not a socket")
    ...
    async def claude_event(self, message: dict[str, Any]) -> None:
        ...
        if kind == "query.started":
            ...
            if isinstance(raw_account_usage, dict):
                ...
                if snapshot:
                    previous = self.claude_account_usage.get("rateLimits")
                    current = (
                        snapshot
                        if raw_account_usage.get("complete") is True
                        else merge_rate_limit_snapshots(previous, snapshot)
                    )
                ...
                elif rate_limit_snapshot(self.claude_account_usage.get("rateLimits")):
                    self.claude_account_usage = {
                        **self.claude_account_usage,
                        "stale": True,
                    }
            ...
            overlay.update(
                {
                    ...
                    "attention": True,
                    "waitType": "approval",
                    "waitDescription": "Permission required in Claude session. Palomar web approval support is not yet available.",
    ...
    def project_claude_session(
        ...
        projected = {
            ...
            "externalLimitation": (
                "Not live-attached. Resume in Palomar to stream, interrupt, or prompt."
                if source == "external"
                else None
            ),
            **overlay,
    ...
    async def codex_event(self, message: dict[str, Any]) -> None:
        ...
        if method == "palomar/runtime/disconnected":
            self.diagnostics.record("runtime.disconnected")
            return
        if method == "palomar/runtime/reconnected":
        ...
        if method == "account/rateLimits/updated":
            ...
            if snapshot:
                ...
                merged = merge_rate_limit_snapshots(previous, snapshot)
                self.account_usage = {
                    "available": True,
                    "rateLimits": merged,
                    "observedAt": int(time.time()),
            ...
            return
        if method in (
            "palomar/approval/requested",
            "palomar/approval/updated",
            "palomar/approval/resolved",
        ):
            ...
            return
        if method in (
            "palomar/input/requested",
            "palomar/input/updated",
            "palomar/input/resolved",
        ):
        ...
        reconciled = params.get("_palomarReconciled") is True
        observed_at = int(time.time())
        advances_activity = event.pop("_palomarAdvancesActivity", False) is True
        reconciled_activity = params.get("_palomarActivityAt")
        ...
        if self.timestamp(reconciled_activity) is not None:
            ...
            replace_activity = (
                params.get("_palomarActivityComplete") is True
        ...
        if self.provider_enabled["codex"]:
            ...
            try:
                ...
                raise UpdateFailure(
                    ...
                    "Palomar could not prove that every session is inactive.",
                ) from error
    ...
    def require_update_not_activating(self) -> None:
        ...
        if isinstance(operation, dict) and operation.get("phase") in ACTIVATION_PHASES:
            raise UpdateFailure(
                "updateActivating",
                "Palomar is activating an update; wait for reconnection before starting new work.",
            )
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            "status": "ok",
            "palomarVersion": PALOMAR_VERSION,
            "protocolVersion": VERSION,
            "palomarConnected": True,
    ...
    def service_status(self) -> dict[str, Any]:
        ...
        return {
            "palomarVersion": PALOMAR_VERSION,
            "palomarReleaseBuild": PALOMAR_RELEASE_BUILD,
            ...
            "codex": {
                ...
                "ownedByPalomar": owns_current_runtime,
                "appServerPid": (
    ...
    async def dispatch(
        ...
        if message_type == "provider.turn.interrupt":
            ...
            if not overlay:
                raise CapabilityError(
                    "External Claude sessions cannot be interrupted because Palomar is not live-attached"
                )
            if overlay.get("state") != "working" or not overlay.get("activeTurnId"):
                raise ValueError("Claude session does not have an active Palomar-owned query")
            await self.require_claude()
            assert self.claude is not None
        ...
        if message_type == "usage.status":
            if self.provider_enabled["codex"]:
                incoming = await self.codex.account_rate_limits()
                snapshot = rate_limit_snapshot(incoming.get("rateLimits"))
                if snapshot:
                    self.account_usage = {
                        "available": True,
                        "rateLimits": snapshot,
                    ...
                    self.state.remember_provider_account_usage("codex", self.account_usage)
                elif rate_limit_snapshot(self.account_usage.get("rateLimits")):
                    self.account_usage = {**self.account_usage, "stale": True}
                    self.state.remember_provider_account_usage("codex", self.account_usage)
...
def claude_prerequisites_available(args: argparse.Namespace) -> bool:
    ...
    claude = os.environ.get("PALOMAR_CLAUDE_EXECUTABLE", "claude")
    return (
    ...
    app = Palomar(
        ...
        provider_installed={
            ...
            "claude-code": executable_available(
                os.environ.get("PALOMAR_CLAUDE_EXECUTABLE", "claude")
            ),
        },
    )
    await app.start()
    sockets = ", ".join(str(sock.getsockname()) for sock in app.server.sockets or [])
    print(f"Palomar listening on {sockets}", flush=True)
    if app.web_server:
        web_sockets = ", ".join(
            str(sock.getsockname()) for sock in app.web_server.sockets
        )
        print(f"Palomar web listening on {web_sockets}", flush=True)
...
def arguments() -> argparse.Namespace:
    config = Path(
        os.environ.get(
            "PALOMAR_CONFIG", "~/.config/palomar/palomar.env"
        )
    ).expanduser()
    load_env(config)
    ...
    parser.add_argument("--host", default=os.environ.get("PALOMAR_HOST", "0.0.0.0"))
    parser.add_argument(
        "--port", type=int, default=int(os.environ.get("PALOMAR_PORT", "8765"))
    )
    parser.add_argument(
        "--web-host", default=os.environ.get("PALOMAR_WEB_HOST", "0.0.0.0")
    )
    parser.add_argument(
        "--web-port", type=int, default=int(os.environ.get("PALOMAR_WEB_PORT", "8766"))
    )
    parser.add_argument(
        "--web-root",
        default=os.environ.get(
            "PALOMAR_WEB_ROOT", str(Path(__file__).resolve().parent / "web")
        ),
    )
    parser.add_argument(
        "--web-origins", default=os.environ.get("PALOMAR_WEB_ORIGINS", "")
    )
    parser.add_argument(
        "--remote-restart",
        action=argparse.BooleanOptionalAction,
        default=environment_flag(os.environ.get("PALOMAR_REMOTE_RESTART", "0")),
    )
    parser.add_argument(
        "--repository-root",
        default=os.environ.get(
            "PALOMAR_REPOSITORY_ROOT", str(Path.home() / "projects")
        ),
    )
    parser.add_argument(
        "--codex",
        default=os.environ.get("PALOMAR_CODEX_EXECUTABLE", "codex"),
    )
    parser.add_argument(
        "--node",
        default=os.environ.get("PALOMAR_NODE_EXECUTABLE", "node"),
    )
    parser.add_argument(
        "--claude-bridge",
        default=os.environ.get(
            "PALOMAR_CLAUDE_BRIDGE",
    ...
    parser.add_argument(
        "--state-directory",
        default=os.environ.get(
            "PALOMAR_STATE_DIRECTORY", str(Path.home() / ".local/state/palomar")
        ),
    )
